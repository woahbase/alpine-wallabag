...
VERSION   ?= $(call get_gh_version,wallabag/wallabag)
...
TESTCMD   := \
	...
	php --version; \
	php-fpm -v; \
	echo "Wallabag version: $$(cat /opt/wallabag/version)"; \
	#
# -- }}}
...
# runtime flags
MOUNTFLAGS := \
	# -v $(CURDIR)/images:/config/www/wallabag/web/assets/images \
	# -v /etc/hosts:/etc/hosts:ro \
	# -v /etc/localtime:/etc/localtime:ro \
	#  # or preseve the whole config dir with
...
OTHERFLAGS := \
	...
	-e PGID=$(PGID) \
	-e PUID=$(PUID) \
	# -e WALLABAG_UPDATE_DEPS=1 \
	# -e WALLABAG_PARAMSFILE=/defaults/parameters.yml \
	# -e WALLABAG_DBINSTALL=true \
	# -e WALLABAG_MIGRATE=true \
	# -e WALLABAG_CLEAR_CACHE=1 \
	# -e WALLABAG_SKIP_CACHEWARMUP=1 \
...
get_config: ## get default config from github
	curl \
		-o $(CURDIR)/root/defaults/parameters.yml \
		-SL https://raw.githubusercontent.com/wallabag/wallabag/refs/heads/master/app/config/parameters.yml.dist
